...
class Image_processing():
    ...
    __allow_type = [".jpg", ".jpeg", ".png"]
    __rootDir = ''

    def image_to_html(self, image_date, image_userid):
        img_stream = ''
        img_local_path='../../license/{}/{}'.format(image_date, image_userid)
    ...
    def convert_images2PDF_one_dir(self, file_dir, save_name=None, filename_sort_fn=None):
        '''
        转换一个目录文件夹下的图片至 PDF
        :param file_dir:
        ...
        for parent, dirnames, filenames in os.walk(file_dir):
            ...
            for file_name in filenames:
                ...
                # 是否图片
                if self.__isAllow_file(file_path):
            ...
            if len(book_pages) > 0:
                ...
                self.__converted(save_name, book_pages, filename_sort_fn)
                endTime = time.clock()
    ...
    def convert_images2PDF_more_dirs(self, dirPath):
        """
        转换一个目录文件夹下的图片至 PDF
        :param file_dir:
        :param filename_sort_fn:
        ...
        for parent, dirnames, filenames in os.walk(dirPath):
            ...
            for filename in filenames:
                ...
                # 检查是否图片
                if self.__isAllow_file(real_filename):
                    # 将图片添加至书本
        ...
        for dirName in __dirs.keys():
            ...
            if dirData['isBook']:
                ...
                beginTime = time.clock()
                self.__converted(os.path.join(dirPath, (dirData['name'] + ".pdf")), dirData['pages'])
                endTime = time.clock()
                print("[*][转换PDF] : 结束. [名称] > [%s] , 耗时 %f s " % (dirName, (endTime - beginTime)))
                index += 1
        print("[*][所有转换完成] : 本次转换检索目录数 %d 个，共转换的PDF %d 本 " % (len(__dirs), index - 1))

    def __isAllow_file(self, filepath):
        """
        是否允许的文件
        :param file:
        :return:
        """
        if filepath and (os.path.splitext(filepath)[1] in self.__allow_type):
            return True
        return False

    def __converted(self, save_book_name, book_pages=[], filename_sort_fn=None):
        """
        开始转换
